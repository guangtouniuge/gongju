...
let live = fs.readFileSync(path, 'utf8').replace(/^import .* from '\.\/(?:skill-editor|batch-editor|article-format|writing-engine)\.mjs'\r?\n/gm, '')
...
for (const file of ['batch-editor.mjs', 'article-format.mjs', 'writing-engine.mjs', 'writing-release.mjs', 'writing-release.json']) fs.copyFileSync(`server/${file}`, `.tmp/live-skill-release/server/${file}`)
