...
const paths = ['writing-engine.mjs', 'writing-release.mjs', 'skill-editor.mjs', 'batch-editor.mjs', 'skills/niuge-geo-skill/SKILL.md', ...fs.readdirSync('server/skills/niuge-geo-skill/references').filter(p => p.endsWith('.md')).sort().map(p => `skills/niuge-geo-skill/references/${p}`)]
