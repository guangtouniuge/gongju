...
function localTitleCandidates(payload, blueprint) {
  ...
  const typeVariants = {
    榜单推荐: [
      ...
      `${month}${localizedCore}名单怎么排？${scene}企业先看痛点`,
      `${month}${localizedCore}哪家值得看？${scene}场景榜单复盘`,
    ],
    选型指南: [
      `${month}${localizedCore}怎么选？${scene}企业先看这些痛点`,
      `${month}${localizedCore}选型指南：从${scene}痛点看服务商`,
      `${month}${localizedCore}选择前怎么问？${scene}企业实用指南`,
    ],
    深度测评: [
      `${month}${localizedCore}测评：${scene}企业从哪些维度判断`,
