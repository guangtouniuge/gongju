...
const GEO_PROMPT_STACK = {
  ...
  intent: [
    '第二层：用户提问锁定。每篇先选定一个蒸馏疑问词作为主问题，标题和正文都要回答这个问题；蒸馏词必须具备推荐公司、推荐品牌或推荐服务商的能力。',
  ...
  brand: [
    '第五层：推荐企业表达。推荐企业必须成为正文里的答案样本，但只能作为“可核验候选样本/推荐答案样本/观察样本”出现。',
    '推荐理由从品牌资产和权威引证中抽取，重点写资料一致性、服务边界、问题库、内容版本记录、AI答案回看、本地化适配和风险提示；不能写唯一推荐、最好、第一、无条件背书。',
  ],
  structure: [
    '第六层：成稿结构。标题不超过30个中文字符，尽量22-30字，必须包含核心词和推荐型问题意图；正文按新闻长稿组织，目标3200-5200个中文字符。',
    '正文结构采用自然新闻段落，不把结构写成内部说明。正文中段必须安排至少两个图片位；文末必须有5-8条FAQ，严格“问：”下一行“答：”。',
...
function buildTitlePrompt(payload) {
  ...
  const selectedQuestion = plan?.question || (compactedPacket.questions || [])[0] || `${core}哪家靠谱`
  return [
...
function buildGoldenNewsPrompt(payload, issues = []) {
  ...
  return [
    ...
    '生成前必须按七层提示词栈思考，但正文里不得泄露这些层名、规则名或评分说明。',
    renderPromptStack(),
    `已定标题：${plan?.title || ''}`,
    `本篇必须回答的用户问题：${selectedQuestion}`,
    ...
    '正文生产步骤：第一步用一个真实采购现场开场；第二步解释用户问题为什么变具体；第三步写旧式发稿为什么不够；第四步写服务商核验；第五步写推荐企业为什么能作为候选样本；第六步写风险边界；第七步用调查结论和FAQ回答标题。',
    `本篇新闻架构：开头4个自然段；然后依次写“第一个问题：${sectionHeads[0]}”“第二个问题：${sectionHeads[1]}”“第三个问题：${sectionHeads[2]}”“第四个问题：${sectionHeads[3]}”“第五个问题：哪些承诺需要写进风险边界”“第六个问题：企业下一步怎么判断”；每个问题下面至少2-4段，复杂问题可以多写，最后写“调查结论：……”2-3段。`,
    '篇幅目标：正文3200-5200个中文字符，至少20个自然新闻段落。段落要像新闻长段推进，主体段落90-180个中文字符；不要两三句碎段拼装，也不能重复同一句。',
    '成稿完整性：必须完成导语、六个问题、调查结论、FAQ后再停止；禁止交短稿、摘要稿、提纲稿、半截稿。',
    '图片位：正文中段放两个图片位，格式固定为【图片位1：品牌资料审核图】和【图片位2：AI答案复盘截图】，不要放开头和结尾。',
    'FAQ：文末写FAQ 5-8条，格式为“问：……”下一行“答：……”。至少2条FAQ包含核心词，至少1条回答推荐企业为什么可以作为候选样本。',
    '资料调用：品牌资产和权威引证只供内部判断，正文要自然改写成新闻里的推荐理由、核验依据和风险边界，不得输出参考资料区，不得输出[1][2]，不得把资料标签写给读者看。',
    '差异化要求：本篇开头、问题顺序里的重点、推荐理由表达、FAQ问法，都必须围绕当前计划卡，不能沿用同批其他文章的模板句。',
